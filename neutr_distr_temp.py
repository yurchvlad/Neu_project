...
for i in range(len(energ_1)):
    ...
    if T_max > b_1:
        result_1 = quad(fn, 3e7, b_1)
        result_2 = quad(fn, b_1, T_max)
        fin_result_1.append((result_1[0] * const_1 + result_2[0] * const_2) * eps_0 ** 2)
    else:
        result_2 = quad(fn, 3e7, T_max)
        fin_result_1.append(result_2[0] * const_2 * eps_0 ** 2)
...
#fin_result = []
#for i in range(1000):
#    eps_0 = eps_0 - (2.132e-10 - 1e-10) / 1000
#    def fn(x):
#        return (k * x / hpl / c) ** 3 * spline(x) / x ** 3 * (Q - eps_0 * x / T_0) ** 2
#    result = quad(fn, 3e7, 1e10)
#    eps_arr.append(eps_0)
